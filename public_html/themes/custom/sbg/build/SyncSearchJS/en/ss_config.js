...
'<form name="search" id="ss-form">' +
	...
	'</div>' +
	// '<div id="ss-keyword-ranking" class="ss-keyword-ranking" style="display: none;"><p class="ss-keyword-ranking-heading">Popular keywords.</p></div>' +
...
'<div class="rn-ss-nav">' +
	// '<div>Results per Page: {{inputHitCount}}</div>' +
	'<div id="ss-navi">' +
		'<div class="ss-navi-left">' +
			// '<span id="ss-query-string">Results: <b>[{{query}}]</b></span>' +
			'<span id="ss-query-string">Results:</span>' +
			'<span id="ss-total-hits"><b>{{total}}</b>Pages</span> ' +
			// '<span id="ss-show-counts"><b>{{start}}</b>Pages &#65374; <b>{{end}}</b>件を表示</span>' +
		'</div>' +
		'<div class="ss-navi-right">' +
			// '<p>Sort by</p>'+
			'<div>'+
				'<span id="ss-sort-date" class="{{sortDateSelected}}"><a href="#">Date</a></span>' +
				'<span id="ss-sort-match" class="{{sortMatchSelected}}"><a href="#">Match</a></span>' +
			'</div>' +
		'</div>' +
	'</div>' +
'</div>';
...
'<span class="ss-page-prev" data-start="{{start}}"><a href="#">Prev</a></span>';
...
'<span class="ss-page-next" data-start="{{start}}"><a href="#">Next</a></span>';
ssConfig.search.templates.pageFirst =
'<span class="ss-page-first" data-start="{{start}}"><a href="#">First</a></span>';
ssConfig.search.templates.pageLast =
'<span class="ss-page-last" data-start="{{start}}"><a href="#">End</a></span>';

ssConfig.search.templates.notfound =
'<div id="ss-not-found">' +
	// '<b>「{{query}}」</b>に一致するページは見つかりませんでした。' +
	'<p>We couldn&#39;t find a page that matches the keyword you&#39;re looking for.</p>'+
...
'<div id="ss-maybe">' +
	'<div id="ss-maybe-message">May be：</div>' +
'</div>';
...
if (ssConfig.search.multiViews != undefined && ssConfig.search.multiViews.length > 0) {
	...
	'<div>' +
		'<div class="ss-multiview-navi">' +
			'<span class="ss-multiview-total-hits"><b>{{total}}</b>Pages</span> ' +
				'<span class="ss-multiview-show-counts"><b>{{start}}</b>Pages &#65374; <b>{{end}}</b>Show</span>' +
		'</div>' +
	'</div>';
...
// '</div>';
	...
	'<div class="ss-multiview-not-found">' +
		'<b>「{{query}}」</b>No pages were found ' +
	'</div>';

	ssConfig.search.multiViews[0].templates.searchLink =
	'<div class="ss-multiview-searchLink">' +
		'<a href="{{searchLink}}">{{viewTitle}}See more search results</a>' +
	'</div>';
